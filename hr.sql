...
--
-- Host: 127.0.0.1
-- Generation Time: Dec 21, 2020 at 03:45 PM
...
-- Database: `hr`
--
CREATE DATABASE IF NOT EXISTS `hr` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `hr`;
...
INSERT INTO `departments` (`id`, `name`, `manager`, `location`) VALUES
...
INSERT INTO `employees` (`id`, `first_name`, `last_name`, `email`, `phone_number`, `hire_date`, `job`, `salary`, `commission_pct`, `manager`, `department`) VALUES
...
(178, 'Kimberely', 'Grant', 'KGRANT', '011.44.1644.429263', '1999-05-24', 'SA_REP', '7000.00', '0.15', 149, 80),
...
(208, 'zxc', 'qwe', 'pamaf18058@95ta.coma', 'qwe', '1987-06-17', 'PU_MAN', '2000.00', '0.00', 100, 90),
(209, 'zxc', 'qwe', 'pamaf18058@95ta.com', 'qwe', '2020-12-16', 'PU_MAN', '2000.00', '0.00', 100, 90);
...
INSERT INTO `job_history` (`employee`, `start_date`, `end_date`, `job`, `department`) VALUES
...
INSERT INTO `users` (`id`, `username`, `password`, `verification_code`, `status`) VALUES
(100, 'kingofsquirrel', '$2a$10$lPKeTvd7gAlhcX4TDfKnY.knTPI2qyxI5gTNy5U6SHk5dX4qlDGdy', NULL, 0),
(208, 'test', '$2a$10$ndGLIcupbrAEAKtENzaGI.LNgkVLcAeZQbWXZJ15Fx7MBtTyHoSde', NULL, 0),
(209, 'testi', '$2a$10$BBP61.VZuaU11G2cAJUjdOjfwLtoS.0s1PU4ud5BEYF7tBq/klDdW', NULL, 0);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `emp_details_view`  AS SELECT `e`.`id` AS `id`, `e`.`job` AS `job`, `e`.`manager` AS `manager`, `e`.`department` AS `department`, `d`.`location` AS `location`, `l`.`country` AS `country`, `e`.`first_name` AS `first_name`, `e`.`last_name` AS `last_name`, `e`.`salary` AS `salary`, `e`.`commission_pct` AS `commission_pct`, `d`.`name` AS `Department Name`, `j`.`id` AS `Job ID`, `l`.`city` AS `city`, `l`.`state_province` AS `state_province`, `c`.`name` AS `name`, `r`.`name` AS `Region Name` FROM (((((`employees` `e` join `departments` `d`) join `jobs` `j`) join `locations` `l`) join `countries` `c`) join `regions` `r`) WHERE `e`.`department` = `d`.`id` AND `d`.`location` = `l`.`id` AND `l`.`country` = `c`.`id` AND `c`.`region` = `r`.`id` AND `j`.`id` = `e`.`job` ;
...
ALTER TABLE `job_history`
  ADD UNIQUE KEY `employee` (`employee`,`start_date`),
  ADD KEY `job` (`job`),
  ADD KEY `department` (`department`);
...
--
ALTER TABLE `locations`
  MODIFY `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3201;
...
--
ALTER TABLE `countries`
  ADD CONSTRAINT `countries_ibfk_1` FOREIGN KEY (`region`) REFERENCES `regions` (`id`);

--
-- Constraints for table `departments`
--
ALTER TABLE `departments`
  ADD CONSTRAINT `departments_ibfk_1` FOREIGN KEY (`location`) REFERENCES `locations` (`id`),
  ADD CONSTRAINT `departments_ibfk_2` FOREIGN KEY (`manager`) REFERENCES `employees` (`id`);

--
-- Constraints for table `employees`
--
ALTER TABLE `employees`
  ADD CONSTRAINT `employees_ibfk_1` FOREIGN KEY (`job`) REFERENCES `jobs` (`id`),
  ADD CONSTRAINT `employees_ibfk_2` FOREIGN KEY (`department`) REFERENCES `departments` (`id`),
  ADD CONSTRAINT `employees_ibfk_3` FOREIGN KEY (`manager`) REFERENCES `employees` (`id`);

--
-- Constraints for table `job_history`
--
ALTER TABLE `job_history`
  ADD CONSTRAINT `job_history_ibfk_1` FOREIGN KEY (`employee`) REFERENCES `employees` (`id`),
  ADD CONSTRAINT `job_history_ibfk_2` FOREIGN KEY (`job`) REFERENCES `jobs` (`id`),
  ADD CONSTRAINT `job_history_ibfk_3` FOREIGN KEY (`department`) REFERENCES `departments` (`id`);
...
--
ALTER TABLE `users`
  ADD CONSTRAINT `users_ibfk_2` FOREIGN KEY (`status`) REFERENCES `user-status` (`id`),
  ADD CONSTRAINT `users_ibfk_3` FOREIGN KEY (`id`) REFERENCES `employees` (`id`);
